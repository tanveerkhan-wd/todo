...
class _HomePageState extends ConsumerState<HomePage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Exported to ${data.name}')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _import() async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Import: pick a .json file')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final groupKey = ValueKey(
      '${groups.length}-${_selectedFilter}-${_searchQuery}-${_selectedListId}-${_selectedTags.length}',
    );
          ...
          if (allTags.isNotEmpty)
            Semantics(
              ...
              child: TagFilterBar(
                ...
                onSelectionChanged: (v) => setState(() => _selectedTags = v),
              ),
            ),

          // Date filter segments + save-preset button
          Semantics(
            label: 'Date filters',
            child: Padding(
                  ...
                  children: [
                    ...
                    if (savedFilters.isNotEmpty)
                      PopupMenuButton<String>(
                        ...
                        onSelected: (id) {
                          final filter =
                              savedFilters.firstWhere((f) => f.id == id);
                          _applyFilterPreset(filter);
                        },
